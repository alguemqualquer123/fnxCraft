...
| Handshake | ✅ |  |
| Entidades sync | ⚠️ parcial | só algumas entidades |
...
| `NullDatabase` | ⚠️ stub | trocar por SQLite pendente |
| `IRepository<T>` | ⚠️ interface | sem implementação real |
...
| Inventário criativo com busca | ❌ | `next-cycle` pendente |
...
- [ ] **DatabaseWorldStorage SQLite** — hoje só `NullDatabase`
- [ ] **Inventário Criativo** (`plans/future/next-cycle.md` #3) — aba `INVENTORY_TAB_CREATIVE` 9×6 paginado + busca
