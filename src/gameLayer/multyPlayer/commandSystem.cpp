...
void initCommandSystem()
{
	...
	//gamerule / hunger / thirst / pvp
	{
		auto gameruleHandler = [](CommandContext &ctx, const std::string &rule, const std::string &val)->std::string{
			auto &s = getServerSettingsReff();
			bool on = (val=="true"||val=="on"||val=="1"||val=="enabled");
			bool off = (val=="false"||val=="off"||val=="0"||val=="disabled");
			if(!on && !off) return "Valor invalido, use true/false ou on/off";
		...
		//gamerule
		{
			...
			ov.args={rule,val};
			def.overloads.push_back(ov);
			def.handler = [gameruleHandler](CommandContext &ctx)->std::string{
				return gameruleHandler(ctx, ctx.arg(0)->str, ctx.arg(1)->str);
			};
			registerCommand(std::move(def));
		}
		//pvp
		{
			CommandDefinition def;
			def.name = "pvp";
			def.description = "Alterna PvP: /pvp <on|off>";
			def.permissionLevel = 2;
			CommandOverload ov;
			CommandArg v; v.name="value"; v.type=CommandArgType::Enum; v.valueNames={"on","off","true","false"};
			ov.args={v};
			def.overloads.push_back(ov);
			def.handler = [gameruleHandler](CommandContext &ctx)->std::string{
				return gameruleHandler(ctx, "pvp", ctx.arg(0)->str);
			};
			registerCommand(std::move(def));
		}
		//hunger
		{
			CommandDefinition def;
			def.name = "hunger";
			def.description = "Alterna fome: /hunger <on|off>";
			def.permissionLevel = 2;
			CommandOverload ov;
			CommandArg v; v.name="value"; v.type=CommandArgType::Enum; v.valueNames={"on","off","true","false"};
			ov.args={v};
			def.overloads.push_back(ov);
			def.handler = [gameruleHandler](CommandContext &ctx)->std::string{
				return gameruleHandler(ctx, "hunger", ctx.arg(0)->str);
			};
			registerCommand(std::move(def));
		}
		//thirst
		{
			CommandDefinition def;
			def.name = "thirst";
			def.description = "Alterna sede: /thirst <on|off>";
			def.permissionLevel = 2;
			CommandOverload ov;
			CommandArg v; v.name="value"; v.type=CommandArgType::Enum; v.valueNames={"on","off","true","false"};
			ov.args={v};
			def.overloads.push_back(ov);
			def.handler = [gameruleHandler](CommandContext &ctx)->std::string{
				return gameruleHandler(ctx, "thirst", ctx.arg(0)->str);
			};
			registerCommand(std::move(def));
		}
	}
	...
	//weather
	{
		CommandDefinition def;
		def.name = "weather";
		def.description = "Changes the weather: /weather <clear|rain|storm|snow>.";
		def.permissionLevel = 0;

		CommandOverload ov;
		CommandArg weatherArg;
		weatherArg.name = "type";
		weatherArg.type = CommandArgType::Enum;
		weatherArg.valueNames = { "clear", "rain", "storm", "snow" };
		ov.args = { weatherArg };
		def.overloads.push_back(ov);
		...
		def.handler = [](CommandContext &ctx) -> std::string
		{
			...
			if (first->str == "clear")
			{
				...
				setWetnessGlobally(0.f);
				return "Weather set to clear.";
			}
			else if (first->str == "rain")
			{
				setWeatherGlobally(1);
				return "Weather set to rain.";
			}
			else if (first->str == "storm")
			{
				setWeatherGlobally(2);
				return "Weather set to storm.";
			}
			else if (first->str == "snow")
			{
				setWeatherGlobally(3);
				return "Weather set to snow.";
			}

			return "Invalid weather type. Use clear, rain, storm, or snow.";
		};
	...
	}
	{
		CommandDefinition def;
		def.name = "flatmobs";
		def.description = "Cria mundo plano 64x64 em y=60 e spawna todos os mobs congelados para visualização.";
		def.permissionLevel = 0;
		CommandOverload ov;
		def.overloads.push_back(ov);
		def.handler = [](CommandContext &ctx) -> std::string
		{
			auto *c = ctx.client;
			glm::ivec3 center = {0,60,0};
			if(c) center = from3DPointToBlock(c->playerData.entity.position);
			center.y = 60;
			g_flatMobsCenter = center;
			g_flatMobsRequested = true;
			g_mobsFrozen = true;
			return "FlatMobs: gerando plataforma plana e todos os mobs congelados em y=61 ao redor de " + std::to_string(center.x) + "," + std::to_string(center.z);
		};
		registerCommand(std::move(def));
	}
	{
		CommandDefinition def;
		def.name = "unfreeze";
		def.description = "Descongela mobs do flat viewer.";
		def.permissionLevel = 0;
		CommandOverload ov;
		def.overloads.push_back(ov);
		def.handler = [](CommandContext &ctx) -> std::string
