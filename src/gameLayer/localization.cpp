...
{
	static_assert(sizeof(s_itemNames) / sizeof(s_itemNames[0]) == lastItem - ItemsStartPoint,
		"itemNames localization array must match the ItemTypes enum count");
	if (itemId >= ItemsStartPoint && itemId < lastItem)
		return pick(s_itemNames[itemId - ItemsStartPoint]);
